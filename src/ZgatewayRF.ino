...
static void RFtoMQTTdiscovery(unsigned long MQTTvalue);
...
static void RFtoMQTTdiscovery(unsigned long MQTTvalue){//on the fly switch creation from received RF values
  char val[11];
  sprintf(val, "%lu", MQTTvalue);
  trc(F("switchRFDiscovery"));
  const char * switchRF[8] = {"switch", val, "", "","",val, "", ""};
